...
import { FacilitiesAndVisitors as FAVType } from '../../../../libs/types'

type Props = {
    data: {
        facilitiesAndVisitors: FAVType
    }
}
...
export const FacilitiesForVisitors = ({ data }: Props) => {
    return (
        <div className='flex flex-col md:flex-row-reverse px-4 md:px-16  justify-around w-full mt-16 gap-12 pb-24'>
            {data && <>
                <div className='relative w-full md:w-[40vw] flex justify-center items-center overflow-hidden z-20'>
                    {data.facilitiesAndVisitors?.videoUrl && (
                        <video
                            loop
                            muted
                            autoPlay
                            className="relative w-full shadow-xl rounded-md z-20"
                        >
                            <source src={data.facilitiesAndVisitors?.videoUrl} type="video/webm" />
